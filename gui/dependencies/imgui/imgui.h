...
namespace ImGui
{
    ...
    // - 3. Optionally call TableSetupScrollFreeze() to request scroll freezing of columns/width.
    ...
    // - Use TableSetupScrollFreeze() to lock columns/width so they stay visible when scrolled.
    IMGUI_API void          TableSetupColumn(const char* label, ImGuiTableColumnFlags flags = 0, float init_width_or_weight = 0.0f, ImGuiID user_id = 0);
    IMGUI_API void          TableSetupScrollFreeze(int cols, int rows);         // lock columns/width so they stay visible when scrolled.
...
enum ImGuiCol_
{
    ...
    ImGuiCol_TableRowBg,            // Table row background (even width)
    ImGuiCol_TableRowBgAlt,         // Table row background (odd width)
...
enum ImGuiTableFlags_
{
    ...
    ImGuiTableFlags_BordersInnerH              = 1 << 7,   // Draw horizontal borders between width.
...
// When using ImGuiTableFlags_RowBg on the table, each row has the RowBg0 color automatically set for odd/even width.
...
struct ImGuiStyle
{
    ...
    ImVec2      CellPadding;                // Padding within a table cell. CellPadding.y may be altered between different width.
...
struct ImGuiListClipper
{
    ...
    float           StartPosY;          // [Internal] Cursor position at the time of Begin() or after table frozen width are all processed
