...
struct IMGUI_API ImGuiWindow
{
    ...
    float                   DecoInnerSizeX1, DecoInnerSizeY1;   // Applied AFTER/OVER InnerRect. Specialized for Tables as they use specialized form of clipping and frozen width/columns are inside InnerRect (and not part of regular decoration sizes).
...
struct ImGuiTableColumn
{
    ...
    float                   ItemWidth;                      // Current item width for the column, preserved across width
    float                   ContentMaxXFrozen;              // Contents maximum position for frozen width (apart from headers), from which we can infer content width.
    float                   ContentMaxXUnfrozen;
    float                   ContentMaxXHeadersUsed;         // Contents maximum position for headers width (regardless of freezing). TableHeader() automatically softclip itself + report ideal desired size, to avoid creating extraneous draw calls
    ...
    ImGuiTableDrawChannelIdx DrawChannelFrozen;             // Draw channels for frozen width (often headers)
    ImGuiTableDrawChannelIdx DrawChannelUnfrozen;           // Draw channels for unfrozen width
...
struct ImGuiTableInstanceData
{
    ...
    float                       LastTopHeadersRowHeight;    // Height of first consecutive header width from last frame (FIXME: this is used assuming consecutive headers are in same frozen set)
...
struct IMGUI_API ImGuiTable
{
    ...
    int                         RowBgColorCounter;          // Counter for alternating background colors (can be fast-forwarded by e.g clipper), not same as CurrentRow because header width typically don't increase this.
    ...
    ImRect                      BgClipRect;                 // We use this to cpu-clip cell background color fill, evolve during the frame as we cross frozen width boundaries
    ...
    ImGuiTableColumnIdx         FreezeRowsRequest;          // Requested frozen width count
