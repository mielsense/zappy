...
// - For large numbers of width, it is recommended you use ImGuiListClipper to submit only visible width.
//   ImGuiListClipper is reliant on the fact that width are of equal height.
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    // Because we cannot safely assert in EndTable() when no width have been created, this seems like our best option.
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // Combine width from regular width + width from headers unless requested not to.
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // visible width, but nav/programmatic scroll would have visible artifacts.)
    ...
    // Else if give no chance to a clipper-savy user to submit width and therefore total contents height used by scrollbar.
    ...
    // This path will only be exercised when sort specs are modified before header width (e.g. init or visibility change)
    if (table->IsSortSpecsDirty && (table->Flags & ImGuiTableFlags_Sortable))
        TableSortSpecsBuild(table);

    // [Part 13] Setup inner window decoration size (for scrolling / nav tracking to properly take account of frozen width/columns)
...
// [Public] Note: for row coloring we use ->RowBgColorCounter which is the same value without counting header width
...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    // End frozen width (when we are past the last frozen row line, teleport cursor and alter clipping rectangle)
...
// based on its position (within frozen width/columns groups or not).
