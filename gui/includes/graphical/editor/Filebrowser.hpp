...
enum ImGuiFileBrowserFlags_ {
    ...
    = 1 << 10, // when entering a new directory, any error will interrupt the process, causing the file browser to fall
               // back to the working directory. with this flag, if an error is caused by a specific item in the
...
ImGui::FileBrowser::Display()
{
    ...
    if (SmallButton("*")) {
        ...
        for (auto &name : selectedFilenames_) {
            const auto it = std::find_if(fileRecords_.begin(), fileRecords_.end(), [&](const FileRecord &record) {
...
inline bool
ImGui::FileBrowser::SetCurrentDirectory(const std::filesystem::path &dir)
{
    try {
...
ImGui::FileBrowser::GetCurrentDirectory() const noexcept
{
...
inline void
ImGui::FileBrowser::SetCurrentDirectoryUncatched(const std::filesystem::path &pwd)
{
...
inline bool
ImGui::FileBrowser::IsExtensionMatched(const std::filesystem::path &_extension) const
{
#ifdef _WIN32
...
inline void
ImGui::FileBrowser::ClearRangeSelectionState()
{
