...
void update_linemate(server_t *server, int linemateQtt)
{
    int j = 0;
    int i = 0;
...
{
    if (server->map[data.client->pos.y][data.client->pos.x].linemateQtt >= 1) {
        server->map[data.client->pos.y][data.client->pos.x].linemateQtt -= 1;
        data.client->inventory->linemate += 1;
...
{
    if (data.client->inventory->linemate < 1)
